...
<template>
	<main class="my-15 mt-25">
		...
		<div class="flex flex-wrap gap-4 mx-12 justify-center">
			<div v-for="actor in uniqueActors" :key="actor.id" class="relative text-white mb-4 group w-52 transition-all duration-300 cursor-pointer hover:z-50 hover:scale-105 justify-items-center" @click="openActor(actor.id)">
				<img v-if="actor.profile_path" :src="`https://image.tmdb.org/t/p/w185${actor.profile_path}`" :alt="actor.name" class="rounded-xl shadow-lg hover:opacity-80 transition min-w-[185px]" />
