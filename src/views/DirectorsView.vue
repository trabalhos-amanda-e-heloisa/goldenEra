...
<template>
	<main class="my-15 mt-25">
		...
		<div class="flex flex-wrap gap-4 mx-12 justify-center">
			<div v-for="director in uniqueDirectors" :key="director.id" class="relative text-white mb-4 group w-52 transition-all duration-300 cursor-pointer hover:z-50 hover:scale-105 justify-items-center" @click="openDirector(director.id)">
				<img v-if="director.profile_path" :src="`https://image.tmdb.org/t/p/w185${director.profile_path}`" :alt="director.name" class="rounded-xl shadow-lg hover:opacity-80 transition" />
				<div v-else class="w-47 h-70 bg-[#0f0f0f] border-2 border-white rounded-xl flex items-center justify-center text-5xl">
