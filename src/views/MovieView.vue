		...
		<div class="flex">
			<div class="ml-20 justify-center w-60">
				...
				<ul class="justify-left flex-wrap list-none p-0">
					<li class="rounded-sm px-1 py-1 w-30 my-2 font-[Sen] text-[#f6a233] hover:text-black hover:bg-[#f6a233]" v-for="genre in genreStore.genres" :key="genre.id" @click="listMovies(genre.id)" :class="{ 'bg-[#f6a233] text-black': genre.id === genreStore.currentGenreId }">{{ genre.name }}
					</li>
				</ul>
				<loading v-model:active="isLoading" is-full-page />
			</div>
			<div class="flex flex-wrap gap-4 mx-12">
				<p v-if="!isLoading && movies.length === 0" class="text-white"> Nenhum filme encontrado com esse gênero.
				</p>
